...
var OpenNewWindowByPost = function (url, paras, target, config, newwindow) {
    newwindow = newwindow != undefined ? newwindow: true;
    var f = '<form method="post" action="' + url + '" target="' + target + '"';
    f += (newwindow ? (' onSubmit="window.open(\'\', \'foo\', \'width=450,height=300,status=yes,resizable=yes,scrollbars=yes\')"'): '')+'>';
    if (paras) {
        if ($.isArray(paras)) {
