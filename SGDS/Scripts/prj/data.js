...
(function (window) {
    ...
    var getFHYFloodSensorStation = function (callback) {
        getData(app.CSgdsRoot + 'WS/FHYBrokerWS.asmx/GetFHYFloodSensorStation', undefined, function (bs) {
            $.get(app.siteRoot + "kml/FSensor.json", function (sd) {
                sd.forEach(s => {
                    if (!s.Sensitivity)
                        return;
                    var b = bs.find(f => {
                        return f.SensorUUID == s.SensorUUID;
                    });
                    if (b)
                        $.extend(b, s);
                });
                var sd = bs.filter(b => { return b.Sensitivity; })
                callback(bs);
            });
