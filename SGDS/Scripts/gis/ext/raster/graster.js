...
(function () {
    var RasterLayer = function (options) {// completed_callback畫完後回call
        ...
        this.displayAllCellInScreen = true; //在Canvas用劃的情況下，如全部資料都在視窗下，在pan時就不重劃
    ...
    RasterLayer.prototype._drawRasterForDrawImage = function () {
        ...
        require(["esri/geometry/Point", "esri/SpatialReference", "esri/geometry/webMercatorUtils", "dojo/dom-construct"], function (Point, SpatialReference, webMercatorUtils, domConstruct) {
            ...
            //var adsadds = current.map.getProjection();
            //var asd = current.map.getProjection().fromLatLngToDivPixel(new google.maps.LatLng(topLeftCorner.y, topLeftCorner.x));
            topLeftCorner = latLng2Point(new google.maps.LatLng(topLeftCorner.y, topLeftCorner.x), current.map);
            
            bottomRightCorner = latLng2Point(new google.maps.LatLng(bottomRightCorner.y, bottomRightCorner.x), current.map);
            ...
            var _mapwidth = current.map.getDiv().width;
            var _mapheight = current.map.getDiv().height;
            if (topLeftCorner.x > _mapwidth)
                return;

            var ssx = (topLeftCorner.x < 0 ? -topLeftCorner.x : 0) / cellWidth;
            var ssy = (topLeftCorner.y < 0 ? -topLeftCorner.y : 0) / cellHeight;
            var ssw = (_mapwidth > bottomRightCorner.x ? bottomRightCorner.x : _mapwidth) / cellWidth - (topLeftCorner.x > 0 ? topLeftCorner.x : 0) / cellWidth;
            var ssh = (_mapheight > bottomRightCorner.y ? bottomRightCorner.y : _mapheight) / cellHeight - (topLeftCorner.y > 0 ? topLeftCorner.y : 0) / cellHeight;

            var tsx = topLeftCorner.x < 0 ? 0 : topLeftCorner.x;
            var tsy = topLeftCorner.y < 0 ? 0 : topLeftCorner.y;
            var tsw = (_mapwidth > bottomRightCorner.x ? bottomRightCorner.x : _mapwidth) - tsx;
            var tsh = (_mapheight > bottomRightCorner.y ? bottomRightCorner.y : _mapheight) - tsy;

            var ctx = current.currentDataCanvas.getContext('2d');
            var bdom = current.map.getDiv();// document.getElementById(current.map.id + "_basemaplayer");
    ...
    RasterLayer.prototype._drawRasterForFillRect = function (zl) {
        ...
        require(["esri/geometry/Point", "esri/SpatialReference", "dojo/dom-construct"], function (Point, SpatialReference, domConstruct) {
            ...
            topLeftCorner = latLng2Point(new google.maps.LatLng(topLeftCorner.y, topLeftCorner.x));
            bottomRightCorner = latLng2Point(new google.maps.LatLng(bottomRightCorner.y, bottomRightCorner.x));
            var dataWidth = bottomRightCorner.x - topLeftCorner.x;
            var dataHeight = bottomRightCorner.y - topLeftCorner.y;
    ...
    function long2px(lon, zoom) { return (Math.floor((lon + 180) / 360 * Math.pow(2, zoom) * 256)); }
    function lat2px(lat, zoom) { return (Math.floor((1 - Math.log(Math.tan(lat * Math.PI / 180) + 1 / Math.cos(lat * Math.PI / 180)) / Math.PI) / 2 * Math.pow(2, zoom) * 256)); }
