        ...
        tableOptions: {//額外提供bootstraptable options
            ...
            detailFormatter: function detailFormatter(index, row) {
                ...
                $.each(fs, function () {
                    ...
                    html.push('<tr><td class="detail-view-field"><b>' + this.title + '</b> </td><td>' + (this.formatter ? this.formatter(v) : (v==undefined?'-':v)) + '</td></tr>')
                });
            ...
            //if (this.settings.tableOptions.url)
            {
                this.settings.queryFilter(_fielter, function (_result) {
                    if (_result !== undefined && $.isArray(_result)) {
                        ...
                        current.$table.bootstrapTable("refresh", { filterColumns: _fielter, pageNumber: 1 }); //透過this.settings.tableOptions.url自動查詢
                });
            }
            else
